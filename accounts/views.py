...
#         auth_code = get_random_string(length=8)
#         password = profile.password
#         user = User.objects.create(
#             auth_code=auth_code, profile=profile, email=profile_email
#         )
#         if not user:
#             return JsonResponse(
...
#         message = f"보내주신 정보를 바탕으로 인증이 진행되었으며, 회원가입이 완료되었습니다. 회원님이 8자리 인증코드는 {auth_code}이며, 비밀번호는 {password}입니다. 보안을 위해 비밀번호를 변경하시기 바랍니다."
#         try:
