...
def predict():
    ...
    with open("./data/CLM_온실가스_MNH_20210515164253.csv") as csvfile:
        ...
        series_decompose = seasonal_decompose(
            csv_co2,
            model="additive",
            period=13,
        )
        ...
        for i in range(48):
            ...
            window_size = 5
            rnn_forecast_s = model_forecast(
                model_series_seasonal, series_seasonal_m[..., np.newaxis], window_size
            )[-1][-1]
            rnn_forecast_r = model_forecast(
                model_series_resid, series_resid_m[..., np.newaxis], window_size
            )[-1][-1]
            rnn_forecast_t = model_forecast(
                model_series_trend, series_trend_m[..., np.newaxis], window_size
            )[-1][-1]
