...
def report_table(file_type, tyear, cate, eg_type):
    ...
    out_df["사용(배출)량"] = out_df["사용(배출)량"].astype(
        "float64")  # string을 float로 변경
    # print(out_df)
...
def total_usems_qnty(year, industry, unit):
    ...
    config = dict(arrowprops=dict(arrowstyle="-"),
    ...
    pic = plt.savefig(f"industry_total_usems_qnty_{year}_{industry}.png")
    import shutil
    shutil.move(f"industry_total_usems_qnty_{year}_{industry}.png",
                f"media/industry_total_usems_qnty_{year}_{industry}.png")
    return result
...
def industry_usems_qnty_statistics(year, industry, usage, unit):
    ...
    import shutil
    shutil.move(f"my_industry_usems_qnty_{year}_{industry}.png",
                f"media/my_industry_usems_qnty_{year}_{industry}.png")
...
def items_usems_qnty_statistics(
    ...
    data = report_table_origin("xml", str(
        year), "업종별", unit)  # 파일형식, 연도, 구분, 에너지/온실가스
    ...
    import shutil
    shutil.move(f"items_usems_qnty_statistics_{year}_{industry}.png",
                f"media/items_usems_qnty_statistics_{year}_{industry}.png")
